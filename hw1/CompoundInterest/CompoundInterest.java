...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
                              int targetYear) {
        double multiplier = 1.00 + (rate / 100);
        return presentValue*Math.pow(multiplier, numYears(targetYear));
    }
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        return futureValue(presentValue, rate, targetYear) * Math.pow(multiplier, numYears(targetYear));
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        double savings = 0;
        double multiplier = 1.00 + rate / 100;
        for (int i = numYears(targetYear); i >= 0; i--) {
            savings += perYear * Math.pow(multiplier, i);
        }
        return savings;
    }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        double multiplier = 1.00 - inflationRate/100;
        return totalSavings(perYear, targetYear, rate) * multiplier;
